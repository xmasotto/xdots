...
(defmacro make-command-on-out (command-prompt command-str)
  (list 'lambda '(command) (list 'interactive (concat "s" command-prompt))
	(list 'let '((original-point (point)))
	      ...
	      (list 'shell-command-on-region '(mark) '(point)
		    (list 'format command-str 'command) nil t)
	      '(setq mark-active nil)
	      '(goto-char original-point))))

(global-set-key (kbd "C-c A")
		(make-command-on-out "awk " "awk '%s'"))
(global-set-key (kbd "C-c S")
		(make-command-on-out "sed " "sed %s"))
(global-set-key (kbd "C-c G")
		(make-command-on-out "grep " "grep %s"))

(defun run-awk-on-out (command)
  (interactive "sAwk: ")
  (let ((original-point (point)))
    (mark-previous-command-out)
    (shell-command-on-region (mark) (point) (concat "awk '" command "'") nil t)
    (setq mark-active nil)
    (goto-char original-point)))
